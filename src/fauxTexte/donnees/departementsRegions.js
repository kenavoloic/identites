...
//export default departementsRegions;
/*
  const _departementsRegions = new Map(liste);
  const departementsRegions = (departement) => _departementsRegions.has(departement.toLowerCase()) ? _departementsRegions.get(departement).regionIso : _departementsRegions.get('33').regionIso;

  const valeurs = Array.from(_departementsRegions.values());
